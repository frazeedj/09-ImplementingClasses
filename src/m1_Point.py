...
class Point(object):
    ...
    def move_to(self,x,y):
        ...
        distance = math.sqrt(((self.x-previous_x)**2)+((self.y-previous_y)**2))
    ...
    def move_by(self,dx,dy):
        self.x = self.x + dx
        self.y = self.y + dy
        previous_x = self.x -dx
        previous_y = self.y -dy
        self.number = self.number+1
        distance = math.sqrt(((self.x-previous_x)**2)+((self.y-previous_y)**2))
        self.total = self.total + distance
    ...
    def get_distance_from_start(self):
        self.distance = math.sqrt(((self.x-self.start_x)**2)+((self.y-self.start_y)**2))
        return self.distance
    ...
    def closer_to(self,p2,p3):
        self.distance1 = math.sqrt(((self.x-p2.x)**2)+((self.y-p2.y)**2)) # distance from p1 to p2
        self.distance2 = math.sqrt(((self.x-p3.x)**2)+((self.y-p3.y)**2)) # distance from p1 to p3
